...
public class BossStage1Controller : MonoBehaviour
{
    ...
    float shootProjectilesWhileLaserAngleMin = 5f;
    float bossMaxMovementSpeedMultiplier = 2.5f;
    float bossMovementSpeedMultiplierTick = 0.8f;

    float laserDanceAttackLasts = 10f;
    ...
    private void FixedUpdate()
    {
        ...
        if (Mathf.Approximately(laserCurrentAngle,laserAngleGoal))
        {
            ...
            if (Mathf.Approximately(laserAngleStart, laserAngleGoal))
            {
                laserAngleGoal = laserAngleEnd;
            }
            if (Mathf.Approximately(laserAngleEnd,laserAngleGoal) && laserDanceAttackTimeSet && Time.time > laserDanceAttackTime)
            {
